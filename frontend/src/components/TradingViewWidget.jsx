...
const TradingViewWidget = ({ symbol }) => {
  ...
  return (
    <div className="relative h-full w-full">
        ...
        <div className="absolute top-2 right-2 z-50">
          {!isEditing ? (
            <button 
              ...
              className="p-2 bg-surface/80 hover:bg-surface text-text-secondary hover:text-text rounded-lg border border-border backdrop-blur-sm shadow-xl transition-all"
              title="Admin: Fix TradingView Symbol"
            >
              <Settings className="w-4 h-4" />
            </button>
          ) : (
            <div className="flex items-center gap-2 p-2 bg-slate-800 rounded-xl border border-primary/50 shadow-2xl backdrop-blur-md">
              <input 
                type="text" 
                value={editValue} 
                onChange={(e) => setEditValue(e.target.value)}
                className="bg-background border border-border text-text text-xs px-3 py-1.5 rounded-lg focus:outline-none focus:border-primary w-40"
      ...
      {widgetError && (
        <div className="absolute inset-0 z-10 flex flex-col items-center justify-center min-h-[500px] bg-background rounded-lg border border-border">
