...
function PostCard({ author, userId, content, timestamp, isOriginal, category }) {
 return (
 <div
 className="bg-surface /50 rounded-2xl p-6"
...
function ReplyCard({ reply, author, index }) {
 ...
 <div className="bg-slate-800/50 /40 rounded-xl px-4 py-3">
...
const TopicDetail = () => {
 ...
 <div className="bg-surface /50 rounded-2xl p-12 text-center text-gray-400">
 Topic non trovato.
 </div>
 </div>
 );
 ...
 {/* ── Reply Form ────────────────────────────────────────────── */}
 <section className="bg-surface /50 rounded-2xl p-6">
 ...
 rows={4}
 className="w-full bg-slate-800/60 /50 rounded-xl px-4 py-3 text-sm text-gray-200 placeholder-gray-500 resize-none focus:outline-none transition-all"
 style={{
